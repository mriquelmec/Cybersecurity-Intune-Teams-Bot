...
Este proyecto es una solución de ciberseguridad automatizada diseñada para organizaciones que utilizan **Microsoft Intune**. El bot identifica dispositivos que no cumplen con las políticas de cumplimiento (*Non-Compliant*), **detecta la razón específica del incumplimiento (ej. cifrado de disco, antivirus)**, activa un **Sincronismo Remoto** para forzar la actualización de políticas y notifica proactivamente al usuario a través de un mensaje de **Microsoft Teams**.

### 🔄 Evolución: De script local a cloud automation
Originalmente, este proyecto nació como un script interactivo que requería autenticación manual del administrador (Device Code Flow). Para llevarlo a un entorno de **Producción Corporativa**, evolucionó a una solución **Serverless** utilizando **GitHub Actions**.

*   **Detección Inteligente:** Ahora identifica qué políticas específicas están fallando para informar al usuario.
...
This project is an automated cybersecurity solution for organizations using **Microsoft Intune**. The bot scans for devices that are **Non-Compliant** with corporate security policies, **identifies the specific reason for non-compliance (e.g., disk encryption, antivirus)**, triggers a **Remote Sync** to force policy updates, and proactively notifies the user via a **Microsoft Teams** message.
...
*   **Intelligent Detection:** Now identifies which specific policies are failing to better inform the user.
...
*   **Microsoft Graph API**: Interaction with Intune (Managed Devices & Compliance Policy States) and Teams (Chat & Messages).
...
- `CLIENT_ID`
- `CLIENT_SECRET`
- `TEST_GROUP_ID` (Optional: to limit the bot's scope to a specific user group).

---

## 📁 Folder structure
- `project.py`: Main bot logic (includes policy reason detection).
